...
- **Changed** ``permission_resources_sql`` hook dispatch to emit one aggregated span per plugin per request rather than one per dispatch. A table page emitted 96 of these spans - 45% of the whole trace - for a hook that performs no I/O: it returns SQL fragments that are concatenated into three actual queries. Span counts for a table page drop from 213 to 123.
- **Fixed** aggregated hook spans reporting a duration spanning their first dispatch to their last, rather than the time actually spent in the hook. Because ``permission_resources_sql`` is dispatched both early in a request and again during template rendering, its aggregate span reported ~47ms of a ~69ms request while consuming under 0.25ms - putting it at the top of "slowest spans" in any tracing UI. The span's duration is now the summed time in the hook, and the first-to-last window is recorded separately as ``datasette.hook.window_ms``.
- **Added** ``datasette.telemetry_registry``, a single source of truth for every span, span attribute and metric Datasette emits. The span and metric reference in :ref:`internals_telemetry` is generated from it with cog, and a conformance test compares it against what real requests actually emit - in both directions - so the reference cannot drift from the code.
- **Added** spans and metrics for the response-building half of a request: template rendering, facet calculation and suggestion, output renderer dispatch and CSV streaming. Previously a slow page whose queries were all fast produced a trace of quick ``db.query`` spans followed by an unexplained gap.
- **Added** OpenTelemetry metrics covering SQL thread pool saturation, per-database write queue depth, open connections, query latency and time-limit interruptions. These answer operational questions that spans structurally cannot - "am I saturating my :ref:`setting_num_sql_threads` threads?" is a level, not an event - and they survive trace sampling. As with spans, core installs no ``MeterProvider``, so there is no cost unless metrics are collected externally. See :ref:`internals_telemetry` for the full list.
- **Added** the :ref:`setting_trace_sql_parameters` setting, defaulting to ``off``, which opts in to recording SQL parameter *values* on spans. ``user`` records them for your databases only; ``all`` includes the internal database, which exports actor identity because permission checks bind the actor as a parameter. Read the warning on that setting before enabling either mode - canned queries using ``_cookie_*`` or ``_header_*`` magic parameters can bind session cookies and ``Authorization`` headers as ordinary SQL parameters.
