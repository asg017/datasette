...
def _otel_provider():
    ...
    `datasette.telemetry.tracer` is a module-level `ProxyTracer`. It only
    caches a concrete tracer once `opentelemetry.trace`'s global provider
    is non-None *at the moment a span is first started* - so as long as
    this fixture runs before any test triggers a `db.query` span (which it
    does, being autouse + session-scoped, so it's part of fixture setup for
    the very first test in the whole run), every span for the rest of the
    process is captured by this exporter. If the SDK isn't installed, do
    nothing: core spans stay no-op `NonRecordingSpan`s and the rest of the
    suite is unaffected.
    """
